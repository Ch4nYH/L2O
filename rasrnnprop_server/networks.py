...
class StandardDeepLSTM(Network):
  ...
  def _build(self, inputs, prev_state, linear_i=-1):
    ...
    if self._preprocess_name is not "fc":
      inputs = self._preprocess(tf.expand_dims(inputs, -1))
    else:
      inputs = tf.nn.elu(self._linear_input(inputs))
    ...
    if linear_i == -1:
      ...
      if not self.init_flag:
        self.init_flag = True
        for i in range(self._num_linear_heads):
          _ = self._linears[i](output)
      final_output = self._linears[linear_i](output)
    if self.tanh_output:
...
class CoordinateWiseDeepLSTM(StandardDeepLSTM):
  ...
  def _build(self, inputs, prev_state, linear_i=-1):
    ...
    output, next_state = build_fn(reshaped_inputs, prev_state, linear_i)
...
class RNNprop(StandardDeepLSTM):
  ...
  def _build(self, m, g, prev_state, linear_i=-1):
