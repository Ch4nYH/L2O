...
class MetaOptimizer(object):
  ...
  def meta_loss(self,
    ...
    def update(net, fx, x, state, mt, vt, t):
      ...
      with tf.name_scope("deltas"):
        deltas, state_next = zip(*[net(m, g, s, linear_i=-1) for m, g, s in zip(mt_tilde, gt_tilde, state)])
    ...
    def time_step_mt(mti):
      def time_step_func(t, loss_array, states, state_mt, state_vt):
        ...
        for si, (k, st, m, v) in enumerate(zip(net_keys, states, state_mt, state_vt)):
          ...
          # net
          delta, state_next_si = net(mt_tilde, gt_tilde, st)  # linear_i=mti + 1)
    ...
    return MetaLoss(loss, update, reset, fx_final, x_final), scale, x, constants, subsets, step, \
           loss_mt, update_mt, reset_multitask, mt_labels, mt_inputs, hess_norm_approx
  ...
  def meta_minimize(self, make_loss, len_unroll, learning_rate=0.01, **kwargs):
    ...
    """
    info, scale, x, constants, subsets, seq_step, loss_mt, update_mt, reset_multitask, mt_labels, mt_inputs, hess_norm_approx = self.meta_loss(make_loss, len_unroll, **kwargs)
    ...
    return MetaStep(step, *info[1:]), scale, x, constants, subsets, seq_step, \
           loss_mt, steps_mt, update_mt, reset_multitask, mt_labels, mt_inputs, hess_norm_approx